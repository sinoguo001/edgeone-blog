...
3. **路由优先级是「静态资源 > 函数」** —— `public/` 下同路径的文件会直接盖住函数。
   所以 `public/` 里**不要**放 `favicon.svg`：站点图标是函数动态生成的（固定品牌蓝 + 站点首字），
   放了就再也变不了。
...
5. **加密文章照常迁** —— 密码字段跟着备份走，迁过来还是加密的；缺该字段的老文章一律当「未加密」处理
...
所以**整站 PV / UV 被整个删掉了**，只留下低频的**文章阅读数**
...
面向服务端的回归测试，共 **23 个脚本、1004 条断言**，**实测 23 / 23 全通过、0 失败**。
...
node .lock-smoke.mjs     # 文章加密：密码闸门 / Cookie 解锁 / 摘要与 RSS 不泄露 / 老数据无密码字段
```

其余同理（`.accent-` / `.bkdel-` / `.blink-` / `.brand-` / `.catslug-` / `.copyright-` /
`.cpui-` / `.email-` / `.emailui-` / `.imp-` / `.links-` / `.mailcustom-` / `.mailui-` /
`.nav-` / `.pages-` / `.permalink-` / `.pubjump-` / `.rootroute-` / `.subcat-`）。
